...
function AnalyticsTab({ clientId }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (typeof payload.accounts === 'string') payload.accounts = JSON.parse(payload.accounts || '[]');
      if (typeof payload.posts === 'string') payload.posts = JSON.parse(payload.posts || '[]');
      if (typeof payload.campaigns === 'string') payload.campaigns = JSON.parse(payload.campaigns || '[]');
      if (typeof payload.alerts === 'string') payload.alerts = JSON.parse(payload.alerts || '[]');
      if (typeof payload.insights === 'string') payload.insights = payload.insights ? JSON.parse(payload.insights) : null;
    } catch (e) {
    ...
    <div className="space-y-5">
      <div style={neu.raised} className="rounded-[28px] p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Btn kind="dark" icon={saving ? Loader2 : Save} onClick={save} disabled={saving}>{saving ? 'Enregistrement…' : 'Enregistrer'}</Btn>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
          {[
            ['total_followers', 'Abonnés totaux', '48 320'], ['followers_delta', 'Évolution abonnés', '+5,2%'],
            ['engagement', 'Engagement', '4,8%'], ['engagement_delta', 'Évolution engagement', '+0,6 pts'],
            ['reach', 'Reach', '284 K'], ['reach_delta', 'Évolution reach', '+12,4%'],
            ['clicks', 'Clics sortants', '1 247'], ['clicks_delta', 'Évolution clics', '-2,1%'],
            ['spent_delta', 'Évolution dépensé', '+8,4%'], ['media_delta', 'Évolution médias', '+12 ce mois'],
          ].map(([key, label, ph]) => (
            <Field key={key} label={label}><Input value={data[key] || ''} onChange={e => upd(key, e.target.value)} placeholder={ph} /></Field>
          ))}
        </div>
      </div>

      <div style={neu.raised} className="rounded-[28px] p-6">
        <div className="text-[11px] uppercase tracking-[0.2em] text-stone-400 font-semibold">Comptes Connectés V2 (JSON)</div>
        <h3 className="text-[18px] tracking-tight mt-1 mb-3" style={SERIF}>Comptes sociaux associés</h3>
        <p className="text-[12px] text-stone-500 mb-3">Format : <code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">{`[{id, platform, account_name, follower_count, total_posts, sync_status, last_sync_at}]`}</code></p>
        <Textarea rows={5} value={typeof data.accounts === 'string' ? data.accounts : JSON.stringify(data.accounts, null, 2)} onChange={e => upd('accounts', e.target.value)} />
      </div>

      <div style={neu.raised} className="rounded-[28px] p-6">
        <div className="text-[11px] uppercase tracking-[0.2em] text-stone-400 font-semibold">Publications V2 (JSON)</div>
        <h3 className="text-[18px] tracking-tight mt-1 mb-3" style={SERIF}>Liste des posts du client</h3>
        <p className="text-[12px] text-stone-500 mb-3">Format : <code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">{`[{id, platform, post_type, published_at, caption, thumbnail_url, hashtags, reach, likes, comments, velocity_score}]`}</code></p>
        <Textarea rows={6} value={typeof data.posts === 'string' ? data.posts : JSON.stringify(data.posts, null, 2)} onChange={e => upd('posts', e.target.value)} />
      </div>

      <div style={neu.raised} className="rounded-[28px] p-6">
        <div className="text-[11px] uppercase tracking-[0.2em] text-stone-400 font-semibold">Campagnes V2 (JSON)</div>
        <h3 className="text-[18px] tracking-tight mt-1 mb-3" style={SERIF}>Campagnes publicitaires actives</h3>
        <p className="text-[12px] text-stone-500 mb-3">Format : <code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">{`[{campaign_id, name, objective, status, budget_total, total_reach, avg_engagement_rate}]`}</code></p>
        <Textarea rows={6} value={typeof data.campaigns === 'string' ? data.campaigns : JSON.stringify(data.campaigns, null, 2)} onChange={e => upd('campaigns', e.target.value)} />
      </div>

      <div style={neu.raised} className="rounded-[28px] p-6">
        <div className="text-[11px] uppercase tracking-[0.2em] text-stone-400 font-semibold">Alertes de performance V2 (JSON)</div>
        <h3 className="text-[18px] tracking-tight mt-1 mb-3" style={SERIF}>Alertes de vitalité / budget</h3>
        <p className="text-[12px] text-stone-500 mb-3">Format : <code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">{`[{id, kind, severity, title, body}]`}</code></p>
        <Textarea rows={4} value={typeof data.alerts === 'string' ? data.alerts : JSON.stringify(data.alerts, null, 2)} onChange={e => upd('alerts', e.target.value)} />
      </div>

      <div style={neu.raised} className="rounded-[28px] p-6">
        <div className="text-[11px] uppercase tracking-[0.2em] text-stone-400 font-semibold">Synthèse IA & Recommandations V2 (JSON)</div>
        <h3 className="text-[18px] tracking-tight mt-1 mb-3" style={SERIF}>Insights hebdomadaires</h3>
        <p className="text-[12px] text-stone-500 mb-3">Format : <code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">{`{headline: "...", body: "...", period_start: "...", recommendations: [{title: "...", body: "..."}]}`}</code></p>
        <Textarea rows={6} value={typeof data.insights === 'string' ? data.insights : JSON.stringify(data.insights, null, 2)} onChange={e => upd('insights', e.target.value)} />
      </div>

      <div style={neu.raised} className="rounded-[28px] p-6 opacity-60">
        <div className="text-[11px] uppercase tracking-[0.2em] text-stone-400 font-semibold">Anciens champs v1 (Pour archive)</div>
        <details className="mt-2 text-[12px] text-stone-500">
          <summary className="cursor-pointer font-medium hover:text-stone-800">Voir les anciens champs JSON (Plateformes, Démographie, etc.)</summary>
          <div className="space-y-4 mt-3">
            <Field label="Plateformes v1"><Textarea rows={4} value={typeof data.platforms === 'string' ? data.platforms : JSON.stringify(data.platforms, null, 2)} onChange={e => upd('platforms', e.target.value)} /></Field>
            <Field label="Démographie v1"><Textarea rows={4} value={typeof data.demographics === 'string' ? data.demographics : JSON.stringify(data.demographics, null, 2)} onChange={e => upd('demographics', e.target.value)} /></Field>
            <Field label="Croissance v1"><Textarea rows={4} value={typeof data.follower_growth === 'string' ? data.follower_growth : JSON.stringify(data.follower_growth, null, 2)} onChange={e => upd('follower_growth', e.target.value)} /></Field>
            <Field label="Engagement quotidien v1"><Textarea rows={4} value={typeof data.engagement_by_day === 'string' ? data.engagement_by_day : JSON.stringify(data.engagement_by_day, null, 2)} onChange={e => upd('engagement_by_day', e.target.value)} /></Field>
            <Field label="Synthèse v1"><Textarea rows={4} value={typeof data.ai_summary === 'string' ? data.ai_summary : JSON.stringify(data.ai_summary, null, 2)} onChange={e => upd('ai_summary', e.target.value)} /></Field>
          </div>
        </details>
      </div>
      ...
      <div className="flex items-center justify-between p-4 rounded-2xl" style={neu.dark}>
        ...
        <Btn kind="dark" icon={saving ? Loader2 : Save} onClick={save} disabled={saving} className="!bg-white !text-stone-900">{saving ? 'Enregistrement…' : 'Enregistrer toutes les analyses'}</Btn>
      </div>
    </div>
  );
}
